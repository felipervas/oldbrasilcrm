...
const Dashboard = () => {
  ...
  const { data: tarefasHoje = [] } = useQuery({
    queryKey: ['dashboard-tarefas-hoje', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      const hoje = new Date().toISOString().split('T')[0];
      const { data } = await supabase
        .from('colaborador_eventos')
        .select('id, titulo, tipo, horario, concluido')
  ...
  // Carregar próximas tarefas pendentes
  const { data: proximasTarefas = [] } = useQuery({
    queryKey: ['dashboard-proximas-tarefas', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      const hoje = new Date().toISOString().split('T')[0];
      const { data } = await supabase
        .from('colaborador_eventos')
        .select('id, titulo, tipo, data, horario, concluido')
        .eq('colaborador_id', user.id)
        .gte('data', hoje)
        .eq('concluido', false)
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-xs">Atividades agendadas para hoje</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            {tarefasHoje.length === 0 ? (
              ...
              <div className="space-y-2">
                {tarefasHoje.map((t: any) => (
                  <div key={t.id} className={`flex items-center gap-3 p-3 rounded-lg border border-slate-100 ${t.concluido ? 'opacity-60' : ''}`}>
                    <span className="text-lg">{getTipoIcon(t.tipo)}</span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium text-slate-900 truncate ${t.concluido ? 'line-through' : ''}`}>{t.titulo}</p>
                      {t.horario && <p className="text-xs text-slate-500">{t.horario}</p>}
                    </div>
                    {t.concluido && <span className="text-xs text-green-600 font-medium">✓</span>}
                  </div>
                ))}
                <Button variant="ghost" size="sm" className="w-full mt-2" onClick={() => navigate("/meu-dia")}>
                  Ver tudo →
                </Button>
              </div>
            )}
        ...
        <CardContent>
          {proximasTarefas.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              <Button variant="ghost" size="sm" className="w-full mt-2" onClick={() => navigate("/tarefas")}>
                Ver todas →
              </Button>
            </div>
          )}
